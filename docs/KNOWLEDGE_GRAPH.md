...
> runbooks y las notas de memoria — en un **grafo navegable** que el agente consulta *antes* de tocar nada.
...
- **Reconstrucción completa** cada vez; ~5 subagentes `general-purpose` en paralelo (por eso `kg-refresh` es
  un skill, no un script: el paso semántico es un paso del agente).
